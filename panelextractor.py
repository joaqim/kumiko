...
# Additional padding in pixels to add to the page for handling unclosed panels.
BODER_PADDING = 10
MASK_SIZE = 15
...
def findComicPanels(image):
    ...
    if isBlackBordered:
        ...
        thresh = cv2.threshold(blurred, maxColorIndex+1,
...
def findBestPanels(image, resized, thresh, ratio, goal, leftIternations, rightIterations):
    ...
    while leftIternations < rightIterations:
        ...
        comicPanels = extractComicPanels(
            resized.copy(), thresh, ratio, midIters, isBright)
    ...
    comicPanels = extractComicPanels(
        resized.copy(), thresh, ratio, bestIteration, isBright)
...
def extractComicPanels(resized, thresh, ratio, iter, isBright):
    ...
    if isBright:
        ...
        # // Dialation ranges from 1 to 4.
        dilation = cv2.dilate(thresh, kernel, iterations=iter)
    ...
    cnts, hierarchy = cv2.findContours(
        eroded.copy(), cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)
    im2 = eroded.copy()
    ...
    for component in zip(cnts, hierarchy):
        ...
        if questionable:
            ...
            if len(approx) <= MAX_CONTOURS_IN_GOOD_PANEL:
                ...
                gridSize = 5
                # TODO Replace cut off point by fraction of the image
                edges, nodes = getEdgesFromContour(
                    approx, gridSize, gridSize, MIN_DISCARD_EDGE_LENGTH)
    ...
    cnts, hierarchy = cv2.findContours(
        thresh.copy(), cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)  # cv2.CHAIN_APPROX_NONE )
    im2 = thresh.copy()
    ...
    print(largePanelArea)
    minAllowedPanelSize = ((originalH * originalW) -
                           (largePanelArea * ratio * ratio)) / MAX_PANELS_IN_PAGE
    ...
    for component in zip(cnts, hierarchy):
        ...
        if frameArea < minAllowedPanelSize:
            print('Skipping Panel - Allowed Area:' +
                  str(minAllowedPanelSize) + ' - ' + str(frameArea))
            continue
...
def generatePanelMetadata(comicPanels):
    ...
    for panelInfo in comicPanels:
        ...
        for cnt in contours:
            shapeEntry = {key_x: int(cnt[0][0]), key_y: int(cnt[0][1])}
